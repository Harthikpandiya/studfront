...
const Certp5copy3 = () => {
  ...
  const handleSearch = async () => {
    try {
      const searchQuery = formData.regNo; // universal input
      ...
      setFormData((prev) => ({
        ...prev,
        ...data,
        file: null, // we don't reassign File object here
...
//   const fetchCourseSuggestions = async (value) => {
//     try {
//       const response = await axios.get(`http://localhost:5000/api/students/courses/search?q=${value}`);
//       setCourseSuggestions(response.data);
//     } catch (error) {
//       console.error("Error fetching course suggestions:", error);
//       setCourseSuggestions([]);
...
//       const fileNameFromDB = data.file || "";
//       const fileName = fileNameFromDB.split("/").pop();
//       setFormData((prev) => ({ ...prev, ...data, file: null, filePath: fileNameFromDB }));
//       setImageName(fileName);
//       setMode("update");
//     } catch (error) {
//       alert("❌ Student not found. Please check the value or try again.");
...
//     try {
//       await axios.post("http://localhost:5000/api/students", data, { headers: { "Content-Type": "multipart/form-data" } });
//       alert("✅ Student submitted successfully");
//       resetForm();
//     } catch (error) {
//       alert("❌ Error submitting student data.");
...
//     try {
//       const data = prepareFormData();
//       await axios.put(`http://localhost:5000/api/students/${formData.regNo}`, data, { headers: { "Content-Type": "multipart/form-data" } });
//       alert("✅ Student updated successfully");
//       resetForm();
//     } catch (error) {
//       alert("❌ Error updating student data.");
...
//     try {
//       await axios.delete(`http://localhost:5000/api/students/${formData.regNo}`);
//       alert("🗑️ Student deleted successfully");
//       resetForm();
//     } catch (error) {
//       alert("❌ Error deleting student data.");
...
//   const resetForm = () => {
//     setFormData({ fullName: "", regNo: "", email: "", courseCode: "", trainerName: "", whatsappNumber: "", date: "", file: null, filePath: "", branch: "" });
//     setImageName("");
//     setPreviewUrl("");
//     setMode("");
//     setStep(1);
